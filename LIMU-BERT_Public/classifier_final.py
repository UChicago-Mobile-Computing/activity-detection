...
from utils_new import get_device, handle_argv \
...
def classify_embeddings(args, data, labels, label_index, training_rate, label_rate, balance=False, method=None):
    ...
    def func_evaluate(label, predicts):
        ...
        with open('predictions_limu_bert.txt', 'w') as out_file:
            for label in output_labels:
                out_file.write(label + "\n")


        print("Saved output as predictions_bert.txt")

        return

    

    #trainer.load('/home/zsarwar/cs/LIMU-BERT-Public/saved/classifier_base_gru_shoaib_20_120/shoaib')
    #trainer.train(func_loss, func_forward, func_evaluate, data_loader_train, data_loader_test, data_loader_vali)
    label_estimate_test = trainer.run(func_forward, func_evaluate, data_loader_test,model_file=os.path.join('saved', 'classifier_base_gru_' + args.dataset  + '_20_120',args.dataset ))
