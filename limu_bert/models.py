    ...
    def __init__(self, cfg, input=None, output=None):
        ...
        for i in range(cfg.num_linear):
            if output is not None and i == cfg.num_linear - 1:
                ...
                self.__setattr__(
                    "lin" + str(i), nn.Linear(cfg.linear_io[i][0], cfg.linear_io[i][1])
                )
...
class ClassifierCNN2D(nn.Module):
    def __init__(self, cfg, output=None):
        ...
        for i in range(cfg.num_linear):
            if i == 0:
                self.__setattr__(
                    "lin" + str(i), nn.Linear(cfg.flat_num, cfg.linear_io[i][1])
                )
...
class BenchmarkDeepSense(nn.Module):
    def __init__(self, cfg, input=None, output=None, num_filter=8):
        ...
        for i in range(self.sensor_num):
            ...
            self.__setattr__(
                "conv" + str(i) + "_2", nn.Conv2d(num_filter, num_filter, (3, 1))
            )
            self.__setattr__(
                "conv" + str(i) + "_3", nn.Conv2d(num_filter, num_filter, (2, 1))
            )
